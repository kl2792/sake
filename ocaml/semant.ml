...
let sl =
  let env' =
    let scope' =
      { S.parent = Some(env.S.scope); S.variables = [] }
    in
...
| S.For(str,(na,nb,nc),stm) ->
  ignore(
    ...
    ignore(require_integer (get_expr env na)); ignore(require_integer (get_expr env nb)); ignore(require_integer (get_expr env nc)); (check_stmt env fsm stm)
